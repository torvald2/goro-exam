  ...
  <div class="bg-gray-200">
    <div class="bg-white mx-14">
      <div class="flex justify-center">
        <div class="flex-1 p-10 py-5 max-w-3xl">
          <div id="paper"></div>
        </div>
        <div class="flex-1 self-center max-w-xs">
          <div class="data-user shadow-lg p-4 rounded">
            <div class="data-name text-2xl">
              <span
                ><span class="font-semibold">{{ getName }}</span></span
              >
            </div>
            <div class="data-date text-lg">
              Дата рождения: {{ birthdayText }}
            </div>
            <div class="data-date text-lg">Время: {{ birthtimeText }}</div>
            <div class="data-date text-lg">Место: {{ getCity }}</div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getFormData: function (data) {
      ...
      var radix = new astrology.Chart("paper", 700, 700).radix(newData);
  ...
  mounted() {
    ...
    var chart = new astrology.Chart("paper", 700, 700).radix(data);
    chart.aspects();

    var date = new Date(this.getBirthDate);
    ...
    this.birthtimeText = date.getHours() + ":" + date.getMinutes();
  },
};
</script>